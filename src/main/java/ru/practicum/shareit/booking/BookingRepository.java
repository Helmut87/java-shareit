...
public interface BookingRepository extends JpaRepository<Booking, Long> {
    ...
    // Найти бронирования по id владельца вещей
    ...
    @Query("SELECT COUNT(b) > 0 FROM Booking b WHERE b.bookerId = :userId AND b.itemId = :itemId AND b.end < :now AND b.status = :status")
    ...
    @Query("SELECT b FROM Booking b WHERE b.itemId = :itemId AND b.start > :now ORDER BY b.start ASC")
